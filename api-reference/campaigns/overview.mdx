...
```
 ...
 + Products      approve/reject    review — you        + metrics       spend
                                   approve/reject
```
...
| Scope             | Grants                                                                          |
| ----------------- | ------------------------------------------------------------------------------- |
| `campaigns:read`  | List and read campaigns, tasks, products, creators, submissions, posts, metrics |
...
The API covers everything you configure in the dashboard's campaign editor: campaign info, visibility, creator targeting, links, example videos, fulfillment products, and the full task brief (captions, ad disclosure, sounds, requirements, review mode, product limits). Campaigns are editable in every status except `completed`, just like the dashboard.

A few dashboard features are intentionally not in the API: creator compensation and billing, paid-ads boosting, organic warm-up, and Canvas campaigns (readable, but their configuration is dashboard-only).
